...
class FaceGraphMeshDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, config, limit_dataset_size=None, single_mode=False):
        ...
        if not single_mode:
            ...
            # self.category = ['car' for _ in range(len(self.items_car))]
            self.items = self.items_chair + self.items_car
            # self.items = self.items_car
        else:
        ...
        self.real_images_preloaded_car, self.real_images_hres_preloaded_car, self.masks_preloaded_car, self.masks_hres_preloaded_car = {}, {}, {}, {}
        self.real_images_preloaded_chair, self.real_images_hres_preloaded_chair, self.masks_preloaded_chair, self.masks_hres_preloaded_chair = {}, {}, {}, {}
        # if config.preload:
    ...
    def __getitem__(self, idx):
        ...
        if category == 'car':
            real_sample, real_sample_hres, real_mask, real_mask_hres, mvp, cam_positions = self.get_image_and_view_car()
        else:
            real_sample, real_sample_hres, real_mask, real_mask_hres, mvp, cam_positions = self.get_image_and_view_chair(selected_item)
    ...
    def get_image_and_view_car(self):
        ...
        for c_i, c_v in zip(image_selections, sampled_view):
            im, im_hres = self.get_real_image(c_i, 'car')
            images.append(im)
            images_hres.append(im_hres)
            ms, ms_hres = self.get_real_mask(c_i, 'car')
            masks.append(ms)
    ...
    def get_image_and_view_chair(self, shape):
        ...
        images, images_hres, masks, masks_hres, cameras, cam_positions = [], [], [], [], [], []
        for c_i, c_v in zip(image_selections, sampled_view):
            im, im_hres = self.get_real_image(self.meta_to_pair(c_i), 'chair')
            images.append(im)
            images_hres.append(im_hres)
            ms, ms_hres = self.get_real_mask(self.meta_to_pair(c_i), 'chair')
            masks.append(ms)
            masks_hres.append(ms_hres)
            azimuth = c_v['azimuth'] + (random.random() - 0.5) * self.camera_noise
            elevation = c_v['elevation'] + (random.random() - 0.5) * self.camera_noise
    ...
    def get_real_image(self, name, category='car'):
        if category == 'car':
            if name not in self.real_images_preloaded_car.keys():
                ...
                return self.real_images_preloaded_car[name], self.real_images_hres_preloaded_car[name]
        else:
            if name not in self.real_images_preloaded_chair.keys():
                return self.process_real_image_chair(self.real_images_chair[name])
            else:
                return self.real_images_preloaded_chair[name], self.real_images_hres_preloaded_chair[name]
    ...
    def get_real_mask(self, name, category='car'):
        if category == 'car':
            if name not in self.masks_preloaded_car.keys():
                ...
                return self.masks_preloaded_car[name], self.masks_hres_preloaded_car[name]
        else:
            if name not in self.masks_preloaded_chair.keys():
                return self.process_real_mask_chair(self.masks_chair[name])
            else:
                return self.masks_preloaded_chair[name], self.masks_hres_preloaded_chair[name]
    ...
    def process_real_image_car(self, path):    
        ...
        pad_size_hres = int(self.image_size_hres * 0.1)
        resize = T.Resize(size=(self.image_size - 2 * pad_size, self.image_size - 2 * pad_size), interpolation=InterpolationMode.LANCZOS)
        resize_hres = T.Resize(size=(self.image_size_hres - 2 * pad_size_hres, self.image_size_hres - 2 * pad_size_hres), interpolation=InterpolationMode.LANCZOS)
    ...
    def process_real_mask_car(self, path):
        ...
        resize_hres = T.Resize(size=(self.image_size_hres - 2 * pad_size_hres, self.image_size_hres - 2 * pad_size_hres), interpolation=InterpolationMode.NEAREST)
        pad = T.Pad(padding=(pad_size, pad_size), fill=1)
        pad_hres = T.Pad(padding=(pad_size_hres, pad_size_hres), fill=1)
        if self.erode:
        ...
        # print("process_real_mask_car: t_mask_hres.shape =", t_mask_hres.shape)
    ...
    def process_real_mask_chair(self, path):
        ...
        # print("process_real_mask_chair: t_mask_hres.shape =", t_mask_hres.shape)
        return t_mask.unsqueeze(0), t_mask_hres.unsqueeze(0)
    ...
    def preload_real_images(self):
        for ri in tqdm(self.real_images_car.keys(), desc='preload'):
            self.real_images_preloaded_car[ri], self.real_images_hres_preloaded_car[ri] = self.process_real_image_car(self.real_images_car[ri])
            self.masks_preloaded_car[ri], self.masks_hres_preloaded_car[ri] = self.process_real_mask_car(self.masks_car[ri])
        for ri in tqdm(self.real_images_chair.keys(), desc='preload'):
            self.real_images_preloaded_chair[ri], self.real_images_hres_preloaded_chair[ri] = self.process_real_image_chair(self.real_images_chair[ri])
            self.masks_preloaded_chair[ri], self.masks_hres_preloaded_chair[ri] = self.process_real_mask_chair(self.masks_chair[ri])
