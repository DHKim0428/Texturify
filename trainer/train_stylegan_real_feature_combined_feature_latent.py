...
class StyleGAN2Trainer(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        f_dict = torch.load("./runs/09111258_StyleGAN23D-Feature-Latent_fast_dev/checkpoints/_epoch=39.ckpt", map_location=self.device)["state_dict"]
    ...
    def forward(self, batch, limit_batch_size=False):
        ...
        z0 = torch.cat((z[0], feature), dim=1)
        z1 = torch.cat((z[1], feature), dim=1)
        # print('z[0]:{0}, feature:{1}, z0:{2}'.format(z[0].shape, feature.shape, z0.shape))
        ######
        w = self.get_mapped_latent((z0, z1), 0.0)
    ...
    def validation_epoch_end(self, _val_step_outputs):
        ...
        with torch.no_grad():
            print("odir_real: {}".format(odir_real))
            print("odir_fake: {}".format(odir_fake))
    ...
    def latent(self, limit_batch_size=False):
        ...
        z1 = torch.randn(batch_size, self.config.latent_dim // 2).to(self.device)
        z2 = torch.randn(batch_size, self.config.latent_dim // 2).to(self.device)
        return z1, z2
...
def main(config):
    trainer = create_trainer("StyleGAN23D-Combined-Feature-latent", config)
