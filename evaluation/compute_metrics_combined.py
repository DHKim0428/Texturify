...
# Car checkpoint
@hydra.main(config_path='../config', config_name='stylegan2_car_test')
def create_interesting_samples_our_gan(config):
    ...
    OUTPUT_DIR_OURS_IMAGES = OUTPUT_DIR / "ours_vis_interesting"


    OUTPUT_DIR_OURS_IMAGES.mkdir(exist_ok=True, parents=True)
    CHECKPOINT = "../checkpoints/original/car/_epoch=449.ckpt"
    CHECKPOINT_EMA = "../checkpoints/original/car/ema_000070649.pth"
    device = torch.device("cuda:0")
    ...
    for iter_idx, batch in enumerate(tqdm(eval_loader)):
        ...
        for z_idx in range(num_latent):
            ...
            # torch.save(z.cpu(), OUTPUT_DIR_OURS_CODES / f"{iter_idx:04d}" / f"{z_idx:04d}.pt")
            for batch_idx in range(fake_render.shape[0]):
                save_image(fake_render[batch_idx], OUTPUT_DIR_OURS_IMAGES / f"{iter_idx}_{batch_idx}_{z_idx}.jpg", value_range=(-1, 1), normalize=True)
