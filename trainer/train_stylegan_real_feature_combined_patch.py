...
from dataset.mesh_real_features_combined_patch import FaceGraphMeshDataset
...
class StyleGAN2Trainer(pl.LightningModule):
    ...
    def g_step(self, batch):
        ...
        # d_patch_input = self.extract_patches_from_tensor(fake_render, 1 - fake_render, self.config.num_patch_per_view, self.config.patch_size)
    ...
    def set_shape_codes(self, batch):
        # print("set_shape_codes: batch['category'] =", batch['category'])
...
@hydra.main(config_path='../config', config_name='stylegan2_combined_patch')
def main(config):
    trainer = create_trainer("StyleGAN23D-Combined-patch", config)
